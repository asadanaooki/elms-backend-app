...
public class FeatureFlagController {
  ...
  /**
   * 指定したキーのフィーチャーフラグを更新する。
   *
   * @param featureFlagKey 更新対象のフィーチャーフラグのキー
   * @param featureFlagUpdateRequest フィーチャーフラグ更新リクエスト
   */
  @Operation(summary = "フィーチャーフラグ更新", description = "指定したキーのフィーチャーフラグの有効状態を更新します")
  @ApiResponses({
  ...
  public void updateFeatureFlag(
      ...
      @RequestBody @Valid FeatureFlagUpdateRequest featureFlagUpdateRequest) {
    FeatureFlagUpdateCommand featureFlagUpdateCommand =
        featureFlagUpdateRequest.toCommand(featureFlagKey);
    featureFlagApplicationService.updateFeatureFlag(featureFlagUpdateCommand);
  }
}
